...
export default function HistorialCompras() {
  ...
  const buscar = async () => {
    setLoadingList(true);
    try {
      const resp = await fetchHistorialCompras({
        desde,
        hasta,
        estado,
      });
  ...
  const verDetalle = async (id) => {
    ...
    try {
      const resp = await fetchCompraDetalle(id);
            ...
            {items.length === 0 ? (
              <tr>
                <td colSpan="5" className="text-center py-4">
                  {loadingList
                    ? "Cargando..."
                    : "No se encontraron compras en ese rango"}
                </td>
              </tr>
            ) : (
              items.map((compra) => (
                <tr key={compra.id}>
                  ...
                  <td className="text-capitalize">{compra.estado}</td>
                  <td>
                    $
